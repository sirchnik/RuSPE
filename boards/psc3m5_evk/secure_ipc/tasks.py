...
def stack_usage(
    ...
    """Run automated GDB stack usage measurement for SPM and services via stack painting."""
    import subprocess
    import time
    ...
    try:
        script_path = REPO_ROOT / "tools" / "debugging" / "gdb_stack_measure_auto.py"
    ...
    return [
        VscodeLaunchTarget(
            **base_conf.to_dict(),
            ...
            executable=f"target/thumbv8m.main-none-eabi/{profile}/{MergedArtifactName.ipc(BOARD, test_nspe_build.NON_SECURE_BOARD).hex}",
            preLaunchCommands=[
        ...
        VscodeLaunchTarget(
            **base_conf.to_dict(),
            ...
            executable=f"target/thumbv8m.main-none-eabi/{profile}/{MergedArtifactName.ipc(BOARD, tock_kernel_build.NON_SECURE_BOARD, has_apps=True).hex}",
            preLaunchCommands=[
